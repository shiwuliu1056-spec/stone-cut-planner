// 文件用途：把排版结果绘制到小程序 Canvas，统一几何与标注规则。
const CANVAS_W = 3000;
const CANVAS_H = 2121;
const PALETTE = [
  "#b9d7e7",
...
function offcutColors(item) {
  return item && item.reusable ? OFFCUT.reusable : OFFCUT.small;
}
...
function groupByDimensions(items) {
  ...
  (items || []).forEach((item) => {
    const rawW = Math.round(Number(item.w) || 0),
      rawH = Math.round(Number(item.h) || 0);
    const w = Math.max(rawW, rawH),
      h = Math.min(rawW, rawH),
      key = `${w}×${h}`;
  ...
  return [...groups.values()].sort(
    (a, b) => b.w * b.h - a.w * a.h || b.w - a.w || b.h - a.h,
  );
}
...
function compactIds(items, placement) {
  const ids = [
    ...new Set(
      (items || [])
        .map((item) =>
          String(
            (placement ? item.id || item.instance : item.id) || "",
          ).replace(/-/g, ""),
        )
        .filter(Boolean),
    ),
  ...
  return ids.length > 4
    ? `${ids.slice(0, 3).join("、")}等${ids.length}项`
    : ids.join("、");
}
...
function arrow(ctx, x, y, direction) {
  ...
  ctx.moveTo(x, y);
  ctx.lineTo(
    x + (direction === "L" ? d : direction === "R" ? -d : -10),
    y + (direction === "U" ? d : direction === "D" ? -d : -10),
  );
  ctx.moveTo(x, y);
  ctx.lineTo(
    x + (direction === "L" ? d : direction === "R" ? -d : 10),
...
function mergeCutSegments(cuts) {
  ...
  groups.forEach((segments, key) => {
    ...
    segments.forEach((segment) => {
      if (union.length && segment[0] <= union[union.length - 1][1] + eps)
        union[union.length - 1][1] = Math.max(
          union[union.length - 1][1],
          segment[1],
        );
...
function drawCutLine(ctx, X, Y, x1, y1, x2, y2, offcuts, placements) {
  const eps = 1e-6,
    ...
    dy = y2 - y1,
    intervals = [];
  const tAt = (value, axis) =>
    axis === "x" ? (value - x1) / (dx || eps) : (value - y1) / (dy || eps);
  (offcuts || []).forEach((r) => {
    let lo = 0,
      hi = 0;
    if (Math.abs(dy) < eps) {
      if (
        y1 <= r.y + eps ||
        y1 >= r.y + r.h - eps ||
        Math.max(x1, x2) <= r.x + eps ||
        Math.min(x1, x2) >= r.x + r.w - eps
      )
        return;
      lo = (r.x - x1) / dx;
      hi = (r.x + r.w - x1) / dx;
      ...
      if (
        x1 <= r.x + eps ||
        x1 >= r.x + r.w - eps ||
        Math.max(y1, y2) <= r.y + eps ||
        Math.min(y1, y2) >= r.y + r.h - eps
      )
        return;
      lo = (r.y - y1) / dy;
      hi = (r.y + r.h - y1) / dy;
    ...
    intervals.push([
      Math.max(0, Math.min(lo, hi)),
      Math.min(1, Math.max(lo, hi)),
    ]);
  });
  (placements || []).forEach((p) => {
    if (Math.abs(dy) < eps) {
      if (Math.abs(y1 - p.y) < eps || Math.abs(y1 - (p.y + p.h)) < eps)
        intervals.push([
          Math.max(0, Math.min(tAt(p.x, "x"), tAt(p.x + p.w, "x"))),
          Math.min(1, Math.max(tAt(p.x, "x"), tAt(p.x + p.w, "x"))),
        ]);
    } else if (Math.abs(x1 - p.x) < eps || Math.abs(x1 - (p.x + p.w)) < eps) {
      intervals.push([
        Math.max(0, Math.min(tAt(p.y, "y"), tAt(p.y + p.h, "y"))),
        Math.min(1, Math.max(tAt(p.y, "y"), tAt(p.y + p.h, "y"))),
      ]);
    }
  });
  ...
  intervals.forEach((iv) => {
    if (blocked.length && iv[0] <= blocked[blocked.length - 1][1] + eps)
      blocked[blocked.length - 1][1] = Math.max(
        blocked[blocked.length - 1][1],
        iv[1],
      );
    else blocked.push(iv);
  });
  let cursor = 0;
  const drawSegment = (a, b) => {
    if (b - a > eps)
      line(ctx, X(x1 + dx * a), Y(y1 + dy * a), X(x1 + dx * b), Y(y1 + dy * b));
  };
  blocked.forEach(([a, b]) => {
    drawSegment(cursor, a);
...
function drawPlacementLabel(ctx, x, y, w, h, placement) {
  ...
  const fontSize = Math.max(
    ...
    Math.min(
      48,
      (horizontal ? h : w) * 0.62,
      (48 * maxWidth) / Math.max(1, ctx.measureText(label).width),
    ),
  );
...
function drawPanelList(ctx, options) {
  ...
  const listTop = panelY + 124,
    listHeight = Math.max(20, sectionH - 144);
  ...
  const fontSize = Math.max(
    16,
    Math.min(baseFontSize, lineHeight * 0.72, baseFontSize * widthFit),
  );
...
function drawSlab(canvas, slab) {
  ...
  (slab.cuts || []).forEach((cut) =>
    placements.forEach((p) => {
      const left = Math.min(cut.x1, cut.x2),
        right = Math.max(cut.x1, cut.x2),
        top = Math.min(cut.y1, cut.y2),
        bottom = Math.max(cut.y1, cut.y2);
      const through =
        cut.x1 === cut.x2
          ? cut.x1 > p.x + 1e-6 &&
            cut.x1 < p.x + p.w - 1e-6 &&
  ...
  const fit = Math.min((CANVAS_W - 120) / W, (CANVAS_H - 120) / H),
    offX = (CANVAS_W - W * fit) / 2,
    offY = (CANVAS_H - H * fit) / 2;
  ...
  offcuts.forEach((item) => {
    const x = X(item.x),
      ...
      w = item.w * scale,
      h = item.h * scale,
      colors = offcutColors(item);
    ctx.fillStyle = colors.fill;
    ctx.fillRect(x, y, w, h);
    ctx.strokeStyle = "#9aa4a9";
    ctx.lineWidth = 3;
    ...
    if (w > 150 && h > 80) {
      ...
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      const font = Math.max(20, Math.min(34, w / 4.5, h / 2.5));
      ctx.font = `700 ${font}px sans-serif`;
      ctx.fillText(item.id || "余料", x + w / 2, y + h / 2 - font * 0.55);
      ctx.font = `500 ${Math.max(18, font * 0.72)}px sans-serif`;
      ctx.fillText(
  ...
  placements.forEach((item) => {
    const x = X(item.x),
      ...
      w = item.w * scale,
      h = item.h * scale,
      moved = movedIds.has(item.instance);
    ctx.globalAlpha = moved ? 0.35 : 1;
    ctx.fillStyle = colorFor(item.id);
    ctx.fillRect(x, y, w, h);
    ctx.strokeStyle = "#334956";
    ctx.lineWidth = 5;
    ctx.setLineDash(moved ? [14, 10] : []);
    ctx.strokeRect(x, y, w, h);
    ctx.setLineDash([]);
    drawPlacementLabel(ctx, x, y, w, h, item);
    ctx.globalAlpha = 1;
  });
  ctx.strokeStyle = "#233844";
  ctx.lineWidth = 6;
  ctx.strokeRect(ox, oy, slabW, slabH);
  const center = ox + slabW / 2;
  ctx.font = "700 44px sans-serif";
  ctx.fillStyle = "#233844";
  ctx.textAlign = "center";
  ctx.fillText(`${slab.id || ""}板 ${slab.index}`, center, oy - 190);
  ctx.font = "500 32px sans-serif";
  ctx.fillStyle = "#65757d";
  ctx.fillText(
    `母板 ${slab.w} × ${slab.h} mm · ${placements.length} 件`,
    center,
    oy - 135,
  );
  ctx.strokeStyle = "#526873";
  ctx.fillStyle = "#526873";
  ctx.lineWidth = 3;
  const dimY = oy - 76;
  line(ctx, ox, dimY, ox + slabW, dimY);
  arrow(ctx, ox, dimY, "L");
  arrow(ctx, ox + slabW, dimY, "R");
  line(ctx, ox, dimY - 24, ox, oy);
  line(ctx, ox + slabW, dimY - 24, ox + slabW, oy);
  ctx.font = "700 36px sans-serif";
  ctx.fillText(`${slab.w} mm`, center, dimY - 20);
  const dimX = ox - 84;
  line(ctx, dimX, oy, dimX, oy + slabH);
  arrow(ctx, dimX, oy, "U");
  arrow(ctx, dimX, oy + slabH, "D");
  line(ctx, dimX - 24, oy, ox, oy);
  line(ctx, dimX - 24, oy + slabH, ox, oy + slabH);
  ctx.save();
  ctx.translate(dimX - 24, oy + slabH / 2);
  ctx.rotate(-Math.PI / 2);
  ctx.fillText(`${slab.h} mm`, 0, -16);
  ctx.restore();
  ctx.lineWidth = 2.4;
  ctx.strokeStyle = "#263b45aa";
  mergeCutSegments(slab.cuts || []).forEach((cut) =>
    drawCutLine(ctx, X, Y, cut.x1, cut.y1, cut.x2, cut.y2, offcuts, placements),
  );
  compactOffcuts.forEach(({ item, x, y }) => {
    const colors = offcutColors(item);
    ctx.beginPath();
    ...
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText(item.id, x, y + 1);
  });
  const panelX = ox + slabW + gap,
    panelY = oy,
    panelH = slabH;
  ctx.fillStyle = "#f7f9f8";
  ctx.strokeStyle = "#ccd5d1";
  ctx.lineWidth = 4;
  ...
  const titleZoom = Math.min(
      2,
      (panelW - 56) /
        Math.max(1, ctx.measureText(`全部余料（${offcuts.length}块）`).width),
    ),
    titleFont = Math.round(40 * titleZoom);
  drawPanelList(ctx, {
    title: `全部下料（${placements.length}件）`,
    items: placementGroups,
    label: (group) => groupLabel(group, true),
    getColor: () => "#8f2f2a",
    panelX,
    panelY,
    panelW,
  ...
  drawPanelList(ctx, {
    ...
    items: offcutGroups,
    label: (group) => groupLabel(group, false),
    getColor: (group) => offcutColors(group).text,
    panelX,
    panelY: dividerY + 8,
    panelW,
...
module.exports = { CANVAS_H, CANVAS_W, drawSlab };
