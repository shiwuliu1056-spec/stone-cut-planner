...
const SLAB_IDS = ['甲', '乙', '丙', '丁', '戊', '己', '庚', '辛', '壬', '癸', '子', '丑', '寅', '卯', '辰', '巳', '午', '未', '申', '酉', '戌', '亥'];
...
function recalculatePartIdsByArea(parts) {
  ...
  parts.map((item, index) => ({ item, index })).sort((a, b) => {
  ...
  }).forEach((entry, rank) => { ranks[entry.index] = rank; });
  return parts.map((item, index) => ({ ...item, id: partId(ranks[index]) }));
}

function normalizeProject(input) {
  const source = input && typeof input === 'object' ? input : {};
  const slabs = Array.isArray(source.slabs) && source.slabs.length
    ? source.slabs.map((item, index) => ({
      id: SLAB_IDS[index] || `板${index + 1}`,
      w: Number(item.w) || 0,
      h: Number(item.h) || 0,
      limit: item.limit === null || item.limit === '' || item.limit === undefined
        ? null : Number(item.limit) || null,
    }))
  ...
  return {
    ...
    algorithm: source.algorithm === 'fast' ? 'fast' : 'standard',
    fastPreset: ['material', 'balanced', 'speed'].includes(source.fastPreset)
      ? source.fastPreset : 'balanced',
  };
}
...
function projectSummary(project) {
  ...
  const totalArea = parts.reduce((sum, item) => (
    sum + ((Number(item.w) || 0) * (Number(item.h) || 0) * (Number(item.qty) || 0)) / 1000000
  ), 0);
  return {
...
module.exports = { defaultProject, loadProject, normalizeProject, projectSummary, saveProject };
