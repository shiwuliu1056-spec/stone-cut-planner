...
function slabGeometryValid(slab) {
  ...
  for (let i = 0; i < rs.length; i += 1) {
    const a = rs[i];
    if (a.x < 0 || a.y < 0 || a.x + a.w > slab.w || a.y + a.h > slab.h)
      return false;
...
test("[P2-1] limit 为小数被拒绝", () => {
  assert.throws(
    () =>
      solve({
        settings: { slabs: [{ id: "甲", w: 2700, h: 1800, limit: 1.5 }] },
        parts: [P("A", 100, 100, 1)],
      }),
...
test("[P2-1] limit 为 0 / 负数被拒绝", () => {
  for (const bad of [0, -1, -3]) {
    assert.throws(
      () =>
        solve({
          settings: { slabs: [{ id: "甲", w: 2700, h: 1800, limit: bad }] },
          parts: [P("A", 100, 100, 1)],
        }),
      (e) => e.code === "INVALID_SOLVE_INPUT",
      `limit=${bad} 应被拒绝`,
...
test("[P2-1] limit 为 null / 空字符串视为不限，可正常排版", () => {
  for (const ok of [null, ""]) {
    const r = solve({
      settings: { slabs: [{ id: "甲", w: 2700, h: 1800, limit: ok }] },
      parts: [P("A", 100, 100, 1)],
    });
...
test("[P2-1] limit 为正整数时严格约束母板数量，不足则报错", () => {
  ...
  assert.throws(
    () =>
      solve({
        settings: { slabs: [{ id: "甲", w: 1000, h: 1000, limit: 1 }] },
        parts: [P("A", 900, 900, 2, false)],
      }),
...
test("[P2-2] 母板尺寸为小数被拒绝", () => {
  assert.throws(
    () =>
      solve({
        settings: { slabs: [{ id: "甲", w: 1000.5, h: 1800, limit: null }] },
        parts: [P("A", 100, 100, 1)],
      }),
    (e) => e.code === "INVALID_SOLVE_INPUT",
  );
...
test("[P2-2] 母板尺寸为 0 或负数被拒绝", () => {
  for (const bad of [
    ...
    assert.throws(
      () =>
        solve({
          settings: { slabs: [{ id: "甲", w: bad.w, h: bad.h, limit: null }] },
          parts: [P("A", 50, 50, 1)],
        }),
      (e) => e.code === "INVALID_SOLVE_INPUT",
    );
...
test("[P2-3] 自定义编号原样保留", () => {
  const r = solve({
    settings: { slabs: [S("CUSTOM", 1000, 1000)] },
    parts: [P("A", 400, 300, 1, true)],
  });
...
test("[R7] cmpMetrics 优先选择连通区域数更少（更连续）的布局", () => {
  ...
  assert.ok(
    cmpMetrics(continuous, broken) < 0,
    "即使边数更多、相邻更好，连续布局仍应胜出",
  );
});
...
test("结果结构符合 API.md /api/solve 契约，且空块统一登记（无 reusable）", () => {
  const r = solve({
    settings: { slabs: [S("甲", 2700, 1800)] },
    parts: [P("A", 1400, 600, 1, true)],
  });
...
test("固定刀片宽度 4mm：中间块留刀口，方向末端块可少 4mm", () => {
  ...
  assert.deepEqual(
    five.plans.A.slabs[0].placements.map((p) => p.x),
    [0, 204, 408, 612, 816],
  );
  ...
  assert.ok(
    six.plans.A.slabs[0].placements.some((p) => p.terminal && p.shortage === 4),
  );
});
...
test("标准排版允许每个方向末端块吸收最多 4mm 损耗", () => {
  const result = solve({
    settings: { slabs: [S("甲", 800, 1600, null)] },
...
test("标准排版：64 件批量输入在移动端可接受时间内完成", {
  timeout: 15000,
}, () => {
  const parts = [
    P("F", 130, 30, 8, true),
  ...
  const result = solve({
    settings: { algorithm: "standard", slabs: [S("甲", 2700, 1800)] },
    parts,
  });
